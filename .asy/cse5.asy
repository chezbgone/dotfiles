	...
	for(int k = 0; flag && (k < 100); ++k){
		...
		if (times.length == 0)
			...
			for(int m = 0; (m < 6) && (times.length == 2); ++m){
				...
				if(length(aa) > 0)
					at = subpath(aa, 0, t - 1/300);
				times = intersect(at, B, LA/10^7);
			}
			l = arclength(a);
			if(l > 0)
				a = subpath(a, reltime(a, arclength(aa)/l), length(a));
			points[count] = point(a, 0);
			a = subpath(a, reltime(a, 1/200), length(a));
			++count;
		}
		if(arclength(a) <= LA/300)
			flag = false;
	}
	return points;
}
...
real d(pair A, pair B){
	if((A == nullpair) || (B == nullpair))
		return -1;
	return sqrt((A.x - B.x)^2 + (A.y - B.y)^2);
}

pair MP(real a = 0, Label L, pair A, int f = pointfontsize, pair B = (0, 0), pen p = pointfontpen){
	if (fullbreak && breakflag)
		return A;
	Label M = L;
	M.s = "\(" + L.s + "\)";
	int ff = f;
...
pair WP(path P, real t = 0.5){
	if((length(P) == -1)||(t > 1.001)||(t < -0.001)){
		if(fullbreak){
			if(length(P) == -1)
				MP("\mbox{There are no points on nullpath}", messageplace, 9);
			else{
				bool tempflag = pathflag;
				pathflag = true;
				D(P, errorpen);
...
pair MA (real a = 0, Label s = '', int f = anglefontsize,
	...
	if(m < 8){
		...
		if(n > 8) n = 8;
		for(int k = 0; k < m; ++k)
			D(shift(A) * scale(r * (1 - k/n)) * P, q);
	}
	if(m > 7 && !(fullbreak && breakflag) && pathflag)
		fill(A--shift(A) * scale(r) * P--cycle, q);
	MP(a, s, D, f, Q, p);
	return A;
}
...
pair MarkPoint(real a = 0, Label L, pair A, int f = pointfontsize, pair B = (0, 0), pen p = pointfontpen) = MP;
