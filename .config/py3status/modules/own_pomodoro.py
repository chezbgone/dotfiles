...
class Py3status:
    ...
    def _time_up(self):
        if self._active:
            self.py3.notify_user("Time is up !", level='warning', title='Pomorodo')
        else:
            self.py3.notify_user("Break #{} time is up !".format(self._break_number), level='warning', title='Pomodoro')
        self._alert = True
        self._advance()
